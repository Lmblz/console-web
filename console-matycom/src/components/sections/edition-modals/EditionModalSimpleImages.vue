<template>
    ...
    <v-dialog width="80%" :retain-focus="false">
        <v-card title="Édition des simples images" class="px-2 pb-2" color="background">
            <v-expansion-panels>
                <v-expansion-panel>
                    <template #title>
                    ...
                    <template #text>
                        ...
                        <v-row class="align-center border-t">
                            <v-checkbox v-model="tempSection.isSlider"
                                        color="primary"
                                        hide-details
                                        style="--v-medium-emphasis-opacity: 1;">
                                <template #label>
                                    <p>Slider d'images en mobile ?</p>
                                </template>
                            </v-checkbox>
                        </v-row>
                    </template>
                </v-expansion-panel>
                <v-expansion-panel v-for="(item, index) in tempSection.images" :key="index">
                    <template #title>
                        <v-row class="ma-0 align-center">
                            <h4>Image n°{{ index + 1 }} - {{ item.picture.alt }}</h4>
            ...
            <v-btn variant="plain" color="primary" text="Ajouter une nouvelle image" @click="tempSection.images.push(new Section().getEmptyElement(tempSection._type))" />
            <v-card-actions>
...
<script setup>
    ...
    function eventSection(event, item) {
        const itemIndex = tempSection.value.images.findIndex(el => el.id == item.id);

        if (event == "up" || event == "down") {
            if ((itemIndex == 0 && event == "up") || (itemIndex == tempSection.value.images.length - 1 && event == "down")) {
            ...
            tempSection.value.images.splice(itemIndex, 1);
            tempSection.value.images.splice(newIndex, 0, item);
        }

        if (event == "delete") {
            if (tempSection.value.images.length == 1) {
                snackbarStore.showSnackbar({
    ...
    function saveSectionEdition() {
        sectionsStore.sectionToEdit.class = "simple-images -more-items-" + tempSection.value.images.length;
